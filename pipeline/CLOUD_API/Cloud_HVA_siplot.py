...
# Paths
# -----------------------------------------------------
ROOT = Path("/Users/raghavasvv/Downloads/Capstone/teamross/capstone3")
RESULTS = ROOT / "results" / "ocean_results"

FILES = {
    "100/250": RESULTS / "ocean_results_250.csv",
    "500": RESULTS / "human_vs_cloud500_metrics_FINAL.csv",
    "1000": RESULTS / "human_vs_cloud1000_metrics.csv"
}
...
for label, path in FILES.items():
    ...
    if label == "100/250":
        for metric in metrics.keys():
            m100 = float(df.loc[df["Metric"] == metric, "Human100_vs_Agent100"])
            m250 = float(df.loc[df["Metric"] == metric, "Human250_vs_Agent250"])
...
# -----------------------------------------------------
# Plot function (Clean, no overlapping box)
# -----------------------------------------------------
def plot_metric(metric_name, ylabel, color):
    ...
    # Add metric labels above bars with auto offset
    ...
    plt.title(
        f"{metric_name} of Human vs Agents (Cloud) Split: 100, 250, 500, 1000",
