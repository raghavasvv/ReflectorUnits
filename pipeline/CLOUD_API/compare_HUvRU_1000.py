...
# -----------------------------------------------------
# File paths (verified for your actual system)
# -----------------------------------------------------
...
def load_human_data(path):
    ...
    participant_count = df["human_id"].nunique() if "human_id" in df.columns else "N/A"
...
# Save results
# -----------------------------------------------------
output_csv = RESULTS_DIR / "human_vs_cloud1000_RUS_metrics.csv"
pd.DataFrame({
    "Metric": metrics.keys(),
    "Human1000_vs_RUS1000": metrics.values()
...
plt.title("1000 Humans vs 1000 RU's (Cloud)", fontsize=14, fontweight="bold", color="#2C3E50")
plt.ylabel("Metric Value", fontsize=11)
plt.grid(axis="y", linestyle="--", alpha=0.6)
plt.tight_layout()

output_png = RESULTS_DIR / "human_vs_cloud1000_RUs_metrics.png"
